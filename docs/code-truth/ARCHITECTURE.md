...
Single source of truth for room geometry. Consumed by Rust (`thermal/geometry.rs::build_rooms/connections/doorways`). Generated by `model/extract_house_inventory.py`, audited by `model/audit_model_dimensions.py`.
...
| Radiator T50 values duplicated in `config.toml` and `thermal_geometry.json` | `config.toml` (analysis.rs), `thermal_geometry.json` (thermal.rs) | Out-of-sync → inconsistent radiator output between analysis and thermal commands |
| Room geometry via `data/canonical/thermal_geometry.json` | `src/thermal/geometry.rs` | `model/audit_model_dimensions.py` verifies wiring |
...
| Radiator T50 in config.toml | Also update `data/canonical/thermal_geometry.json` (thermal.rs reads from there) |
