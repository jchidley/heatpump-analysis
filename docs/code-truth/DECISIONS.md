...
**What**: On shutdown, restore `Z1OpMode=auto` + `Hc1HeatCurve=0.55`. VRC 700 resumes timer control.

**Where**: `src/bin/adaptive-heating-mvp.rs::restore_baseline()`, `model/adaptive-heating-mvp.toml` `[baseline]`

### D8: Z1OpMode=night (SP=19) during V2 operation

**Status:** active (since Phase 1a, 1 Apr 2026)

**What**: On startup, set `Z1OpMode=night` (value 3). VRC 700 uses `Z1NightTemp` (19°C) permanently. Disables Optimum Start, day/night transitions, timer interference.
...
### OQ3: HwcMode (eco/normal) writable via eBUS?
...
### OQ4: Eco/normal crossover temperature?
...
### OQ5: Does `CurrentCompressorUtil = -57` mean anything useful?
