...
| `tsdb.rs` | ~450 | PostgreSQL/TimescaleDB query routing and shared timestamp helpers |
| `display.rs` | 1014 | CLI output, **equilibrium solver**, **MWT bisection**, **control table generation** |
| `report.rs` | 44 | Table printer and RMSE |
| `history.rs` | ~2259 | Heating/DHW history reconstruction from PostgreSQL/TimescaleDB. Comfort miss detection (clipped to 07:00–23:00 waking hours), DHW overlap detection, controller event extraction |
...
| `model/thermal-config.toml` | `src/thermal/config.rs` | PostgreSQL settings, test nights, calibration bounds |
...
| Shared thermal TimescaleDB readers | `src/thermal/tsdb.rs` |
