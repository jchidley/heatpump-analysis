...
14. **Conservatory as measured boundary condition**: stop predicting conservatory temperature. Instead, use the measured conservatory sensor temp as a boundary condition (like outside temp) for computing heat flows to/from adjacent rooms (kitchen, hall via doorways; leather via wall). This eliminates conservatory ACH/solar/wind sensitivity from the model entirely. Radiator output into conservatory is still computable from MWT but represents heat "lost" to the buffer zone, not useful house heating. The conservatory sensor effectively becomes a second outside-temp input — warmer than ambient, colder than heated rooms, measured not predicted. This should also improve kitchen + hall accuracy since their doorway exchange with conservatory will use real temps instead of poorly predicted ones. Future SE solar array still valuable for understanding conservatory behaviour but no longer needed for the room model.
