//! CRC-8 calculation using polynomial 0x9B.
//!
//! Per eBUS spec section 5.7, CRC is calculated over the *extended*
//! (byte-stuffed) transmission sequence.

/// CRC-8 lookup table for polynomial 0x9B
/// (x⁸ + x⁷ + x⁴ + x³ + x¹ + 1).
#[rustfmt::skip]
...
/// One step of the CRC-8 calculation.
///
/// Chain calls: `let crc = calc_crc(b2, calc_crc(b1, 0));`
#[inline]
pub fn calc_crc(byte: u8, init: u8) -> u8 {
    CRC_TABLE[init as usize] ^ byte
}

/// Calculate CRC-8 over an entire byte slice.
pub fn crc_bytes(data: &[u8]) -> u8 {
    data.iter().fold(0u8, |acc, &b| calc_crc(b, acc))
}
...
mod tests {
    ...
    #[test]
    fn crc_step_init_zero() {
        // calc_crc(byte, 0) == byte because table[0] == 0
        assert_eq!(calc_crc(0x77, 0x00), 0x77);
    }

    #[test]
    fn crc_step_init_one() {
        // table[1] = 0x9b, so calc_crc(0, 1) = 0x9b ^ 0x00 = 0x9b
        assert_eq!(calc_crc(0x00, 0x01), 0x9b);
    }

    #[test]
    fn crc_chain_two_bytes() {
        let mut crc = 0u8;
        crc = calc_crc(0x10, crc);
        crc = calc_crc(0x08, crc);
        assert_eq!(crc, 0x3a);
    }

    #[test]
    fn crc_full_sequence() {
        // Known telegram: 10 08 b5 11 02 03 00 -> CRC 0x1e
        let data = [0x10, 0x08, 0xb5, 0x11, 0x02, 0x03, 0x00];
        assert_eq!(crc_bytes(&data), 0x1e);
    }
}
