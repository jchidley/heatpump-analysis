...
The next actions have moved to the shared tracker in `~/github/energy-hub/lat.md/tsdb-migration.md`: keep the deployed controller path reconciled with source until the cutover window, then retire Telegraf's v2 output, remove the Grafana v2 datasource, stop/remove the InfluxDB v2 container, and archive the v2 data volume.
