...
Uses forecast temperature, solar irradiance, and humidity. `ForecastCache` refreshes every 3600s. When compressor is not actively heating, falls back to `default_delta_t_c` (4.0°C) instead of live flow-return ΔT — prevents target oscillation when flow ≈ return. At the warm low-load end, when forecast outside is within 5°C of the VRC setpoint (19°C) and required flow is ≤28.5°C, the outer loop no longer inverts the heat-curve formula because that mapping becomes ill-conditioned; it seeds the known-safe baseline curve (0.55) and suppresses inner-loop flow chasing so the 60-second loop cannot ramp to curve 4.00 for a 26–27°C target. During active heating, the outer loop also avoids ratcheting the curve back down when the model seed is below the current curve but `Hc1ActualFlowTempDesired` is still materially below `target_flow_c`; this stops the 15-minute loop from fighting the 60-second feedback loop while the VRC is still converging. Model calculation runs every tick even during DHW charges — only eBUS writes are suppressed. Action is logged as `dhw_active` with full model fields so the controller is never blind.
...
When `target_flow_c` is `None`, the inner loop does nothing. This covers both overnight coast and warm low-load fallback ticks where the outer loop deliberately uses the baseline curve without chasing an ill-conditioned target flow.
...
During any Cosy window, DHW launches unconditionally if the cylinder needs charging — battery state is irrelevant because grid electricity is at its cheapest. Outside Cosy windows, the controller checks the `energy-hub` headroom signal `emon/tesla/discretionary_headroom_to_next_cosy_kWh` before launching. Currently only the overnight battery slot (00:00–04:00) is actively scheduled; the other non-Cosy gaps (07:00–13:00, 16:00–22:00) have no software DHW scheduling and rely on VRC 700 timer fallback rails. That signal is `available_above_reserve − projected_base_load_to_next_cosy`; positive means spare capacity, negative means deficit. The gate is `headroom >= dhw_event_kwh` (eco 1.9 kWh, normal 2.4 kWh). The controller also predicts T1 at 07:00, reads `hmu HwcMode` for the eco/normal kWh estimate, and now uses `dhw.remaining_litres` plus the latest `dhw_capacity.recommended_full_litres` as an independent practical-volume trigger. Remaining litres are capped by the recommended full capacity before being compared with slot budgets (morning 89L, afternoon 72L, overnight 62L), so a warm T1 can no longer mask an already-depleted cylinder. Timer windows are still maintained as fallback rails by [[src/bin/adaptive-heating-mvp.rs#sync_morning_dhw_timer]]. Raw Powerwall SoC / power topics remain useful for observability but the scheduling decision uses the headroom signal, not raw telemetry.

The headroom signal publishes every 10s and is part of the live control path. The energy-hub value is now explicitly current-SoC-based and carries projection metadata; during Cosy slots that means it does not extrapolate Tesla-controlled charging. The controller still ignores headroom during Cosy slots.

Timer/tariff boundary rule: if an imported evening window arrives as `22:00–00:00`, the controller must normalize that to `22:00–23:59` for same-day runtime matching and to `22:00–-:-` for VRC 700 timer writes. The write-time conversion is the hard safety boundary: raw `00:00` must never be written to `HwcTimer_*`. `00:00` is not a valid end-of-day timer encoding on eBUS.
...
- **Headroom during Cosy**: derivation must use current SoC only because Tesla controls charging; metadata should make that basis explicit. Controller ignores headroom during Cosy. Lesson: verify MQTT topics arrive after energy-hub changes.
...
| `model/adaptive-heating-mvp.toml` | Config (eBUS, TSDB settings, Cosy windows, baseline, tuning) |
