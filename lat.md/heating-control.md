...
The headroom signal went live on 5 Apr 2026 (deployed to emonpi ~22:20 BST, verified end-to-end through MQTT → Telegraf → TSDB ingest → controller). Publishing every 10s with no gaps since deployment. Legacy Influx-backed verification was the original proof path; final PostgreSQL-only completion remains part of [[tsdb-migration]]. Note: the energy-hub headroom value is unreliable during Cosy windows because it projects drain from current SoC without accounting for active grid charging — this doesn't matter because the controller ignores it during Cosy slots.
