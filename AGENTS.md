...
- **Overnight**: 19°C setback 00:00-04:00. DHW windows: 05:30-07:00, 13:00-15:00, 22:00-00:00. Phase 2 replaces with T1-based DHW decisions: preferred 22:00 charge + 04:00 top-up if needed. See `docs/heating-plan.md` and `docs/dhw-plan.md`.
...
- **V2 overnight planner validated** (3-4 Apr 2026): Two consecutive clean nights, Leather ≥20.7°C, zero comfort misses. Outside 9-11°C (mild). Not yet tested below 5°C. **Bug found**: planner never actually coasted — missing `break` in binary search meant it always picked coast=0 (heat immediately). Fixed in e11cbd6. First real coast night pending.
- **Model vs empirical mismatch**: τ=15h model vs ~45h empirical (35 segments), K=7500 vs ~20600 empirical (27 segments). Both errors reinforced "always heat all night". The empirical τ reflects the real physics: heating stops in a warm house, same initial condition whether DHW or overnight coast. Model τ=15h is wrong. See `docs/heating-plan.md` § Empirical vs model parameters.
...
- **Phase 2 deployed** (2 Apr 2026): Overnight planner replaces fixed coast. Cooling simulation (τ=15h), reheat estimation (7500 W per °C/h), latest-start binary search. Cold night override (<2°C: maintain 19.5°C). Awaiting first overnight run for validation.
...
- **V2 heating control** (`docs/heating-plan.md`): Model-predictive control. Outer loop (15 min): live thermal solver (`bisect_mwt_for_room`) + Open-Meteo forecast → target flow temp. Inner loop (1 min): nudge curve until `Hc1ActualFlowTempDesired` matches target — VRC 700 treated as black box. `Z1OpMode=night` on startup (flat 19°C setpoint, no timer/Optimum Start). Restore `Z1OpMode=auto` on shutdown. Overnight planner calculates latest heating start time for 20°C by 07:00; cold nights (<2°C) maintain heating.
