...
- **Overnight**: controller coasts (curve 0.10) after 23:00 until calculated preheat start (latest-safe, τ=50h). At 10°C outside: coasts ~6h, Leather dips ~1°C, preheat starts ~05:00. DHW windows: 05:30-07:00, 13:00-15:00, 22:00-00:00. See `docs/heating-plan.md` and `docs/dhw-plan.md`.
...
- **Empirical τ**: 50h from daytime DHW segments (n=53). First coast night (4–5 Apr) was **not a genuine coast** — curve 0.10 at SP=19 still produces ~21°C flow, HP cycled on/off (BC flow 711–860 L/h). Can't derive overnight τ from confounded data. K=7500 in code, empirical K≈20,600 — not yet tested. See `docs/heating-plan.md`.
- **⚠ Curve 0.10 ≠ zero output.** `Hc1MinFlowTempDesired`=20°C floor means the VRC 700 always demands ≥20°C flow. Need `Z1OpMode=off` or lower MinFlow to achieve genuine coast.
